...
description: >-
  ...
  its GitHub repo using the build-once + ArgoCD direct-deploy pattern. Use this
  whenever the user wants to deploy, ship, stand up, onboard, or roll out a
  service/repo to the Saras GKE infra, set up its CI/CD, add it to ArgoCD,
  create per-env Cloud SQL databases for it, or promote a build dev→test→prod —
  e.g. "deploy https://github.com/sarasanalytics-com/{svc} to dev", "stand up
  {svc} on test/QA", "ship this service to prod", "deploy the latest build to
  QA", "promote this build to prod". Pass a GitHub repo URL and it deploys (or
  promotes) the service. Also trigger on mentions of build-once image promotion,
  autoship/saras-ai-gateway-style deployment, the argo-deployment branch,
  dev-saras/test-saras/saras namespaces, or the dev→test→prod deploy flow, even
  if "skill" isn't said.
---

# Deploy a service to Saras GKE (build-once + ArgoCD direct-deploy)
...
per-env Helm values file off the repo's `argo-deployment` branch **with
auto-sync on**. Promotion is a **direct ArgoCD deploy**: to move a build to an
env, commit that build's image tag into the env's values file and nudge ArgoCD
to sync — it applies in seconds, with no separate promotion engine to drift or
deadlock.

> **Why not Kargo?** Earlier versions of this skill drove dev→test→prod with
> Kargo Stages. That model **deadlocks on repeat deploys** in this single-branch
> setup: a successful deploy doesn't reliably advance the Stage, so the panel
> reads stale state and the next promotion is blocked. Pinning the app revision
> (`updateTargetRevision`) only half-fixes it. It has been replaced by the
> direct-deploy model below. See `references/promotion.md` for the mechanism and
> `references/gotchas.md` for the full post-mortem.

Read `references/infra.md` for the concrete cluster/registry/DB/DNS facts,
`references/promotion.md` for the deploy/promote mechanism, and
`references/gotchas.md` for the failure modes that will bite you — all are
...
- **Needs a database?** (optional) — many services do; detect from `.env.example`/code. Some services need a **second** DB connection (e.g. a shared read DB) on a different instance — that's a separate proxy sidecar, see gotchas.
...
- **Does it need a DB?** Check `.env.example` for `DATABASE_URL`/`DB_*` and a `cloudSqlProxy` block in the chart. A second DB URL pointing at a different host means a second proxy sidecar (see gotchas).
...
4. **Values** — write `helm/{env}-values.yaml` (template in `templates/values.yaml.tmpl`): image repo+tag (**keep the tag quoted** so numeric run-ids aren't parsed as numbers/floats), `cloudSqlProxy.instanceConnectionName` for the env instance, `ingress.host`, `secretRef`, and a pod-anti-affinity keyed to `{svc}-{env}`. Commit it to the `argo-deployment` branch (the existing `deploy-dev` flow only touches `dev-values.yaml`, so committing other env values doesn't collide).
5. **ArgoCD app** — create `{svc}-{env}` (template in `templates/argocd-app.yaml.tmpl`): the env's `project`, destination `server`+`namespace`, source `path: helm` + `valueFiles: [{env}-values.yaml]` on `argo-deployment`, `syncPolicy.automated.selfHeal: true` + `CreateNamespace=true`. Apply to the **argocd** cluster. **That auto-sync is the deploy engine** — it's what applies every later promotion, so it must stay on.
...
### 3. Wire promotion (direct ArgoCD deploy)

Promotion is just **moving the build-once tag into the next env's values file and
letting ArgoCD apply it** — no Kargo, no promotion CRs. Full mechanism and a
backend snippet are in `references/promotion.md`; the essentials:

- **Each env's ArgoCD app already auto-syncs** the `argo-deployment` branch (step 2.5). That is the deploy engine — nothing else applies the change.
- **Deploy build X to env E** = commit X's tag into `helm/{E}-values.yaml` on `argo-deployment` (GitHub Contents API: read the file, replace the `tag:` line **keeping its quotes**, commit; skip if unchanged), then **nudge** the app to sync now instead of at the next poll: `kubectl annotate application {svc}-{E} -n argocd argocd.argoproj.io/refresh=hard --overwrite` (or the API equivalent). It's live in ~30–40s.
- **Never write `dev-values.yaml` from here** — the GitHub `deploy-dev` workflow owns dev. Direct-deploy owns `test-values.yaml` / `prod-values.yaml` only.
- **Gate prod on QA**: only allow a prod deploy of a build that is *currently the image running in test/QA* (compare the test app's live image to the tag).
- **Read live state from the ArgoCD apps**, never from a side ledger: each env's current image (`status.summary.images`), health (`status.health.status`) and sync (`status.sync.status`) come straight from its `Application`. That's why a deploy can't look falsely "stuck."
- **RBAC**: gate the QA and prod deploy actions behind explicit roles (admins always allowed). The SA driving the nudge needs `get`+`patch` on Applications in the `argocd` namespace.

### 4. Validate

Confirm the pod is `Running` (2/2 with the proxy, 3/3 with a second DB proxy),
`/health` returns `db: connected`, logs show schema init and the expected
active/passive line, and the public URL responds end-to-end (DNS → LB → Traefik
→ pod). For prod, validate while still passive before any activation.
...
- `references/infra.md` — clusters, projects, registries, Cloud SQL instances, namespaces, ArgoCD projects, ingress + DNS targets, host/naming conventions.
- `references/promotion.md` — the direct-deploy mechanism (commit tag + ArgoCD nudge), prod-gating, reading live state from ArgoCD, and why Kargo Stages were dropped.
- `references/gotchas.md` — the failure modes (Kargo-stage deadlock, Postgres schema perms, Cloud SQL import ownership, proxy auth, secondary-DB sidecar, Cloudflare API quirks, env-naming footgun, and more).
- `templates/` — `values.yaml.tmpl`, `argocd-app.yaml.tmpl`.
